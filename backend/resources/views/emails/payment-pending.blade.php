...
<head>
    ...
    <title>Pembayaran Sedang Diverifikasi</title>
    <style>
        ...
        .button { display: inline-block; padding: 14px 28px; background-color: #f59e0b; color: #ffffff !important; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px; box-shadow: 0 4px 6px -1px rgba(245, 158, 11, 0.2); }
...
<body>
    <div class="wrapper">
        <div class="container">
            <div class="header">
                <h1>Bukti Bayar Diterima</h1>
            </div>
            <div class="content">
                <p class="intro">Halo {{ $order->user->name }},</p>
                <p>Kami telah menerima unggahan bukti pembayaran Anda untuk pesanan nomor <strong>#{{ $order->order_number }}</strong> senilai **Rp {{ number_format($order->total_price_actual ?? $order->total_price, 0, ',', '.') }}**.</p>
                
                <p>Tim administrasi kami sedang melakukan verifikasi dan pencocokan mutasi rekening secara manual. Proses verifikasi biasanya memakan waktu 5-15 menit.</p>
                
                <p>Kami akan mengirimkan email notifikasi kembali segera setelah pembayaran Anda berhasil dikonfirmasi.</p>

                <div class="button-container">
                    <a href="{{ config('app.url') }}/invoice/{{ $order->invoice_token }}" class="button" target="_blank">Lihat Invoice Saya</a>
                </div>
            </div>
            <div class="footer">
