...
const ServicesPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F8FAFC] via-[#F1F5F9] to-[#E2E8F0] font-sans flex flex-col relative overflow-hidden">
      ...
      <div className="relative pt-24 pb-8 px-4 z-10 flex flex-col items-center animate-[fadeIn_0.6s_ease-out]">
        <h1 className="text-3xl md:text-5xl font-extrabold text-gray-900 tracking-tight mb-3 text-center">
          Pilih <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2563EB] to-[#71C9CE]">Durasi & Layanan</span>
        </h1>
        <p className="max-w-xl text-gray-500 text-sm font-medium leading-relaxed mb-8 text-center">
        ...
        <div className="w-full max-w-4xl mx-auto mb-10">
          <div className="flex items-center justify-center mb-5 px-2">
            <h2 className="text-sm md:text-base font-extrabold text-gray-800 uppercase tracking-widest flex items-center gap-2">
              ...
              <label 
                ...
                className={`relative flex flex-col items-center p-4 rounded-2xl border-2 cursor-pointer transition-all duration-300 ${
                  selectedDuration === duration.id
                    ? 'border-[#2563EB] bg-[#2563EB]/5 shadow-md shadow-[#2563EB]/10 scale-[1.02]'
                    : 'border-white bg-white hover:border-[#A6E3E9] hover:bg-gray-50 shadow-sm'
                }`}
              >
                <input 
                  type="radio" 
                ...
                <span className={`text-xs font-extrabold uppercase tracking-wider mb-1 ${
                  selectedDuration === duration.id ? 'text-[#2563EB]' : 'text-gray-400'
                }`}>
                  {duration.badge}
                </span>
                <span className={`text-lg font-black tracking-tight ${
                  selectedDuration === duration.id ? 'text-gray-900' : 'text-gray-700'
                }`}>
                  {duration.label}
                </span>
                ...
                {/* Visual Checkmark */}
                {selectedDuration === duration.id && (
                  <div className="absolute -top-2 -right-2 w-6 h-6 bg-[#2563EB] text-white rounded-full flex items-center justify-center shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 p-1.5 bg-white rounded-2xl shadow-sm border border-gray-100">
          <button
      ...
      <main className="flex-grow max-w-6xl mx-auto px-6 pb-24 w-full z-10">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <span className="text-sm font-bold text-[#2563EB] animate-pulse">Menghitung estimasi harga...</span>
          </div>
        ) : (
          /* Cards Grid */
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredServices.map((service, index) => {
                ...
                return (
                  <div
                    key={service.id}
                    className="group bg-white rounded-3xl border border-[#A6E3E9]/30 hover:border-[#71C9CE] p-6 flex flex-col h-full shadow-lg hover:shadow-[#2563EB]/15 hover:-translate-y-1.5 transition-all duration-300"
                    style={{ animationDelay: `${index * 100}ms`, animationFillMode: 'both', animationName: 'fadeIn' }}
                  >
                    {/* Top Badges */}
                    <div className="flex items-center justify-between mb-6">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#E3FDFD] text-[#2563EB] text-[10px] font-bold uppercase tracking-wider">
                        {service.categoryName}
                      </span>
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-50 text-orange-600 border border-orange-100 text-[10px] font-bold uppercase tracking-wider transition-colors duration-300">
                    ...
                    {/* Description */}
                    <p className="text-sm text-gray-500 font-medium leading-relaxed flex-grow mb-6">
                      {service.description}
                    </p>
                    ...
                    <div className="mt-auto pt-5 border-t border-gray-100 flex items-end justify-between">
                      <div>
                        ...
                        <div className="flex items-baseline gap-1">
                          <span className="text-2xl font-black text-[#2563EB] transition-all duration-300">
                      ...
                      <Link
                        to="/login"
                        className="bg-gray-900 hover:bg-[#2563EB] text-white p-3 rounded-xl transition-colors duration-300 shadow-md shadow-gray-900/10 hover:shadow-[#2563EB]/30"
