...
const Orders = () => {
  ...
  const [activeFilter, setActiveFilter] = useState('all'); // all, active, completed, unpaid
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white p-6 rounded-card border border-slate-200/60 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-brandText tracking-tight">Riwayat Pesanan</h1>
        ...
        <Link
          to="/orders/new"
          className="w-full sm:w-auto px-5 py-2.5 bg-primary hover:bg-primary-hover text-white text-xs font-bold rounded-btn shadow-sm hover:shadow text-center transition-all duration-200"
        >
          + Order Baru
        </Link>
      </div>
      ...
      {filteredOrders.length === 0 ? (
        <div className="bg-white p-16 rounded-card border border-slate-200/60 shadow-sm text-center space-y-4">
          <div className="w-16 h-16 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center text-2xl mx-auto">
            🧺
          </div>
          <div className="space-y-1">
            <h3 className="text-sm font-bold text-brandText">Tidak Ada Pesanan</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredOrders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className="bg-white p-6 rounded-card border border-slate-200/60 shadow-sm hover:border-slate-300 hover:shadow-md transition-smooth flex flex-col justify-between gap-5"
              >
                ...
                <div className="flex justify-between items-center border-t border-slate-50 pt-4">
                  ...
                  <Link
                    ...
                    className="px-3.5 py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold rounded-btn transition-colors"
                  >
                    Detail Pesanan
                  </Link>
                </div>
