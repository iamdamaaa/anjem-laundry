...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-8 font-sans">
      {/* Top Banner Greeting */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 sm:p-8 rounded-card border border-slate-200/60 shadow-sm gap-6">
        <div className="space-y-1">
          <h1 className="text-2xl sm:text-3xl font-black text-brandText tracking-tight">
        ...
        <Link
          to="/orders/new"
          className="w-full sm:w-auto px-6 py-3.5 rounded-btn bg-primary hover:bg-primary-hover text-white font-extrabold text-sm text-center shadow-lg shadow-blue-500/10 hover:shadow-blue-500/25 transition-all duration-200"
        >
          🧺 Buat Pesanan Baru
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-card border border-slate-200/60 shadow-sm flex items-center gap-5 hover:border-slate-300 transition-colors duration-200">
          ...
          <div>
            ...
            <span className="text-2xl font-black text-brandText mt-1 block">{totalOrders}</span>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-black text-brandText tracking-tight">Pesanan Terbaru</h2>
        ...
        {orders.length === 0 ? (
          <div className="bg-white p-12 rounded-card border border-slate-200/60 shadow-sm text-center space-y-4">
            <div className="w-16 h-16 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center text-2xl mx-auto">
              🧺
            </div>
            <div className="space-y-1">
              <h3 className="text-sm font-bold text-brandText">Belum Ada Pesanan</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {orders.slice(0, 4).map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className="bg-white p-5 rounded-card border border-slate-200/60 shadow-sm hover:border-slate-300 transition-colors flex flex-col justify-between gap-4"
                >
