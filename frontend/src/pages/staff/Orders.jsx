...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchStaffOrders = async () => {
      try {
        // Fetch staff orders using admin endpoint (it lists all or pre-filtered by assigned staff)
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white p-6 rounded-card border border-slate-200/60 shadow-sm">
        <h1 className="text-2xl font-black text-brandText tracking-tight">Tugas Laundry Saya</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {orders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className="bg-white p-5 rounded-card border border-slate-200/60 shadow-sm hover:shadow-md transition-smooth flex flex-col justify-between gap-4"
              >
                ...
                <div className="flex justify-between items-center border-t border-slate-50 pt-3">
                  ...
                  <Link
                    ...
                    className="px-4 py-2 bg-teal-800 hover:bg-teal-700 text-white text-xs font-bold rounded-btn transition-colors"
                  >
                    Kerjakan Tugas
                  </Link>
                </div>
